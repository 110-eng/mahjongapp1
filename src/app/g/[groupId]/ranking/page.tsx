...
export default async function RankingPage({
  ...
  return (
    <div className="space-y-5">
      <h1 className="font-serif text-lg font-bold text-ink-900">ランキング</h1>

      <div className="flex gap-1 rounded-full bg-ink-400/10 p-1 text-sm font-medium">
        <Link
          href={`/g/${groupId}/ranking?period=quarter`}
        ...
        <Link
          href={`/g/${groupId}/ranking?period=season`}
          className={`flex-1 rounded-full py-2 text-center transition-colors ${
      ...
      {mode === "quarter" && (
        <div className="flex gap-2">
          {quarters.map((q) => (
            <Link
              key={q.quarter}
              href={`/g/${groupId}/ranking?period=quarter&quarter=${q.quarter}`}
      ...
      <p className="text-xs text-ink-400">
        {mode === "season"
          ? seasonLabel(seasonYear, group.seasonStartMonth)
